...
export default class Hl7Model {
  ...
  private static createMessage(message: Hl7IMessage): void {
    ...
    Hl7Model.createFile(
      "messages",
      `${message.name.toLowerCase()}Message`,
      content
    );
  }
  ...
  private static createFields(segmentType: string, fields: Hl7IFields): void {
    ...
    Hl7Model.createFile(
      "fields",
      `${segmentType.toLowerCase()}Fields`,
      content
    );
  }
  ...
  private static buildMessage(message: Hl7IMessage): string {
    let imports = "";
    let messageProps = "";
    Object.entries(Object.getOwnPropertyDescriptors(message)).forEach(
      (message) => {
  ...
  private static buildSegment(segment: Hl7ISegment): string {
    let segmentProps = "";
    Object.entries(Object.getOwnPropertyDescriptors(segment)).forEach(
      (segmentProp) => {
